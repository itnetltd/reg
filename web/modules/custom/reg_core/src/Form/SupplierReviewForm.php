...
final class SupplierReviewForm extends FormBase {
  ...
  protected AccountProxyInterface $regCurrentUser;
  protected FileUrlGeneratorInterface $fileUrlGenerator;
  protected MailManagerInterface $mailManager;
  protected LanguageManagerInterface $regLanguageManager;

  public function __construct(SupplierManager $supplier_manager, AccountProxyInterface $current_user, FileUrlGeneratorInterface $file_url_generator, MailManagerInterface $mail_manager, LanguageManagerInterface $language_manager) {
    $this->supplierManager = $supplier_manager;
    $this->regCurrentUser = $current_user;
  ...
  public function buildForm(array $form, FormStateInterface $form_state, ?NodeInterface $supplier = NULL): array {
    ...
    $form['decision'] = ['#type' => 'radios', '#title' => $this->t('Decision'), '#required' => TRUE, '#options' => ['approved' => $this->t('Approve / Reactivate'), 'request_update' => $this->t('Request update'), 'rejected' => $this->t('Reject'), 'suspended' => $this->t('Suspend'), 'archived' => $this->t('Archive')]];
  ...
  public function validateForm(array &$form, FormStateInterface $form_state): void { $decision = $form_state->getValue('decision'); if ($decision === 'approved') { $supplier = \Drupal::entityTypeManager()->getStorage('node')->load($form_state->get('supplier_id')); if ((string) $supplier->get('field_reg_document_scan_status')->value !== 'clean') $form_state->setErrorByName('decision', $this->t('Approval is blocked until all private registration documents have passed malware scanning.')); } elseif (!trim((string) $form_state->getValue('reason'))) $form_state->setErrorByName('reason', $this->t('A reason is required for this decision.')); }
  public function submitForm(array &$form, FormStateInterface $form_state): void {
    $storage = \Drupal::entityTypeManager()->getStorage('node');
    $supplier = $storage->load($form_state->get('supplier_id'));
    $decision = (string) $form_state->getValue('decision');
    $reason = trim((string) $form_state->getValue('reason'));
    $old_status = (string) $supplier->get('field_reg_supplier_status')->value;
    $supplier->set('field_reg_supplier_status', $decision)->set('field_reg_review_reason', $reason)->set('field_reg_internal_review_note', trim((string) $form_state->getValue('internal_note')))->setNewRevision(TRUE);
    $supplier->setRevisionLogMessage('Supplier status changed from ' . $old_status . ' to ' . $decision . ($reason ? ': ' . $reason : '.'));
    $supplier->save();
    $membership_ids = $storage->getQuery()->accessCheck(FALSE)->condition('type', 'reg_supplier_membership')->condition('field_reg_supplier_org.target_id', $supplier->id())->execute();
    foreach ($storage->loadMultiple($membership_ids) as $membership) {
      $account = $membership->get('field_reg_member_user')->entity;
      if (!$account) continue;
      if ($decision === 'approved') {
        $account->addRole('reg_supplier')->removeRole('reg_supplier_pending');
      }
      else {
        $account->addRole('reg_supplier_pending')->removeRole('reg_supplier');
      }
      $account->save();
    }
    $detail = $old_status . ' → ' . $decision . ($reason ? '; ' . $reason : '');
    reg_core_supplier_audit('status_changed', (int) $supplier->id(), (int) $this->regCurrentUser->id(), $detail);
    $email = (string) $supplier->get('field_reg_email')->value;
    if ($email) $this->mailManager->mail('reg_core', 'supplier_review', $email, $this->regLanguageManager->getCurrentLanguage()->getId(), ['organization' => $supplier->label(), 'decision' => ucfirst(str_replace('_', ' ', $decision)), 'reason' => $reason]);
