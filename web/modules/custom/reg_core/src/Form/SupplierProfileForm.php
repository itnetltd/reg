...
final class SupplierProfileForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state): array {
    ...
    if ($org && in_array((string) $v($org, 'field_reg_supplier_status'), ['rejected', 'suspended', 'expired'], TRUE)) { $form['locked'] = ['#markup' => '<div class="messages messages--warning"><p>' . $this->t('This organization profile is not open for self-service changes. Contact REG Procurement and quote your company registration number.') . '</p></div>']; $form['#attached']['library'][] = 'reg_core/supplier_portal'; return $form; }
    ...
    $form['company_type'] = ['#type' => 'select', '#title' => $this->t('Company type'), '#required' => TRUE, '#options' => ['company' => $this->t('Company'), 'partnership' => $this->t('Partnership'), 'consultant' => $this->t('Individual consultant'), 'ngo' => $this->t('NGO / institution'), 'international' => $this->t('International company'), 'other' => $this->t('Other approved type')], '#default_value' => $v($org, 'field_reg_company_type', 'company')];
    ...
    $form['documents'] = ['#type' => 'managed_file', '#title' => $this->t('Registration and compliance documents'), '#description' => $this->t('PDF, DOC, or DOCX. Maximum 25 MB per file. Files remain private and must pass malware scanning.'), '#multiple' => TRUE, '#upload_location' => 'private://supplier-registration/' . $this->regCurrentUser->id(), '#upload_validators' => ['FileExtension' => ['extensions' => 'pdf doc docx'], 'FileSizeLimit' => ['fileLimit' => 26214400]], '#default_value' => $v($org, 'field_reg_private_documents', [])];
  ...
  public function validateForm(array &$form, FormStateInterface $form_state): void {
    ...
    if ($tin && $this->supplierManager->tinExists($tin, (int) ($org?->id() ?: 0))) $form_state->setErrorByName('tin', $this->t('This TIN is already associated with an active supplier record. Contact REG Procurement if you need access to that organization.'));
    $allowed_mimes = ['application/pdf', 'application/msword', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document']; foreach (array_filter($form_state->getValue('documents') ?: []) as $fid) { $file = $this->regEntityTypeManager->getStorage('file')->load($fid); if ($file instanceof FileInterface && !in_array($file->getMimeType(), $allowed_mimes, TRUE)) $form_state->setErrorByName('documents', $this->t('A document has an unsupported file type. Upload only a genuine PDF, DOC, or DOCX file.')); }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state): void {
    ...
    $current_status = (string) $org->get('field_reg_supplier_status')->value; $next_status = $review ? 'pending' : ($current_status === 'verified' ? 'correction' : ($current_status ?: 'draft'));
